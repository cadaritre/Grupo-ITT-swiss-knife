...
def solve_rigid_registration(
    ...
    """Solve source -> target with a scale-preserving Kabsch transform.

    Unit factors only normalize explicitly selected input units to metres. No scale
    is estimated from the controls, which prevents a bad correspondence from
    silently stretching a survey.
    """
    ...
    source_center = source_m.mean(axis=0)
    target_center = target_m.mean(axis=0)
    covariance = (source_m - source_center).T @ (target_m - target_center)
    ...
    residuals = np.linalg.norm(predicted - target_m, axis=1)
...
def write_registration_report(
    ...
    target = Path(path)
    payload = {
        "schema": "grupo-itt.pointcloud-registration.v2",
        ...
        "note": "La matriz transforma coordenadas crudas de la nube de dron al sistema local, en metros, de la nube de escáner.",
    }
...
def run_merge_worker(request: dict, queue=None, cancel_event=None) -> None:
    ...
    try:
        ...
        write_registration_report(report_path, result, request, output)
        _queue_message(
            queue, "done", output_path=str(output), report_path=str(report_path),
            result=asdict(result), point_count=completed,
        )
