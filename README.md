...
- Registro planimétrico restringido a giro sobre Z y traslación XY. Las elevaciones absolutas del escáner y del dron se convierten a metros, pero nunca se inclinan ni reciben traslación vertical; cualquier diferencia de cota se reporta como control de calidad independiente.
...
- Visor avanzado Open3D en un proceso gráfico independiente: renderizado acelerado por GPU, hasta 250 000 puntos por nube, RGB original cuando existe, coloración por origen o elevación, tamaño de punto, fondos, ejes y vistas isométrica/superior/frontal/lateral. Permite alternar entre escáner, dron original, dron ajustado y superposición registrada sin bloquear Tkinter.
- Open3D se distribuye como `Visor3DGrupoITT.exe`, un componente opcional desmarcado por defecto en el árbol del instalador. La aplicación principal no contiene ni carga Open3D; si el componente no está instalado, únicamente se desactiva la apertura del visor GPU y siguen disponibles el registro, la vista ligera y la fusión LAZ. Puede agregarse después ejecutando el mismo MSI y eligiendo **Cambiar**.
- Auditoría al terminar cada LAZ: compara puntos declarados del escáner y el dron contra los puntos escritos y el encabezado final; la diferencia queda visible en pantalla y guardada en el JSON de registro.
